...
def _call_mods(features_batch, model):
    ...
    for vi, vsfeatures in enumerate(testloader):
        # only 1 loop
        vkmer, vbase_means, vbase_stds, vbase_signal_lens = vsfeatures
        if use_cuda:
            vkmer = vkmer.cuda()
...
def _call_mods_q(model_path, features_batch_q, pred_str_q, success_file, args):
    ...
    model = SeqEncoderClassifier(args.seq_len, args.d_model, args.d_ff, args.n_head, args.layer_num,
...
def main():
    ...
    p_call.add_argument("--model_path", "-m", action="store", type=str, required=True,
                        help="file path of the trained model (.pkl)")
    ...
    p_call.add_argument('--dropout_rate', type=float, default=0.1, required=False)
    ...
    p_call.add_argument('--d_model', type=int, default=512, required=False)
    p_call.add_argument('--d_ff', type=int, default=1024, required=False)
