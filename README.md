...
For the example data (deepsignal-plant extracts 13-mer-seq and 13*16-signal features of each CpG motif in reads by default. Note that the value of *--corrected_group* must be the same as that of *--corrected-group* in tombo.):
```bash
...
To call modifications, either the extracted-feature file or **the raw fast5 files (recommend)** can be used as input.
...
A modification-frequency file can be generated by the script [scripts/call_modification_frequency.py](https://github.com/PengNi/deepsignal-plant/blob/master/scripts/call_modification_frequency.py) with the call_mods file as input:
```bash
# call 5mCpG for instance

# output in tsv format
python /path/to/deepsignal-plant/scripts/call_modification_frequency.py --input_path fast5s.CG.call_mods.tsv --result_file fast5s.CG.call_mods.frequency.tsv
# output in bedmethyl format
python /path/to/deepsignal-plant/scripts/call_modification_frequency.py --input_path fast5s.CG.call_mods.tsv --result_file fast5s.CG.call_mods.frequency.bed --bed

```

The modification_frequency file can be either saved in bedmethyl format (by setting `--bed`) or be saved as a tab-delimited text file in the following format by default:
   - **chrom**: the chromosome name
   - **pos**:   0-based position of the targeted base in the chromosome
   - **strand**:    +/-, the aligned strand of the read to the reference
   - **pos_in_strand**: 0-based position of the targeted base in the aligned strand of the chromosome
